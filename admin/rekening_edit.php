...
if ((isset($_POST["ftambah"])) && ($_POST["ftambah"] == "y")) {
    ...
    $sql_cek = $con->query("SELECT * FROM rekening WHERE bank='$bank' AND kd_rekening!='$kd_rekening' ");
    $row_cek = $sql_cek->fetch(PDO::FETCH_LAZY);
    $trow_cek = $sql_cek->rowCount();

    if (empty($trow_cek)) { // bila data tidak ada
    	// proses simpan
	    $con->exec("UPDATE rekening SET bank='$bank', no_rek='$no_rek' WHERE kd_rekening='$kd_rekening' ");
	    // pesan berhasil
...
$sql = $con->query("SELECT * FROM rekening WHERE kd_rekening='$kd_rekening' ");
			                                ...
			                                <form method="POST" enctype="multipart/form-data">
			                                    <div class="form-group">
			                                        ...
			                                        <div class="input-group col-xs-9">
			                                            <input type="text" class="form-control" name="bank" required autocomplete="off" value="<?php echo $row['bank'] ?>">
			                                    ...
			                                    <div class="form-group">
			                                        ...
			                                        <div class="input-group col-xs-9">
			                                            <input type="text" class="form-control" name="no_rek" required autocomplete="off" onKeyPress="return goodchars(event,'0123456789',this)" value="<?php echo $row['no_rek'] ?>">
			                                            <span class="input-group-addon danger" style="display: none;"></span>
			                                        </div>
			                                    </div>
			                                    ...
			                                    <input type="hidden" name="kd_rekening" value="<?php echo $kd_rekening; ?>" />
			                                </form>
			                            </div>
			                        </div><!-- /.row -->
				                </div> <!-- /.panel body -->
