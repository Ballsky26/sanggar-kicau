...
if ((isset($_POST["ftambah"])) && ($_POST["ftambah"] == "y")) {
    ...
    $sql_cek = $con->query("SELECT * FROM halaman WHERE nama_halaman='$nama_halaman' AND kd_halaman!='$kd_halaman' ");
    $row_cek = $sql_cek->fetch(PDO::FETCH_LAZY);
    $trow_cek = $sql_cek->rowCount();

    if (empty($trow_cek)) { // bila data tidak ada
    	// proses simpan
	    $con->exec("UPDATE halaman SET nama_halaman='$nama_halaman', isi_halaman='$isi_halaman', admin='$admin' WHERE kd_halaman='$kd_halaman' ");
	    // pesan berhasil
...
$sql = $con->query("SELECT * FROM halaman WHERE kd_halaman='$kd_halaman' ");
			                                ...
			                                <form method="POST" enctype="multipart/form-data">
			                                    <div class="form-group">
			                                        ...
			                                        <div class="input-group col-xs-4">
			                                            <input type="text" class="form-control" name="nama_halaman" required autocomplete="off" value="<?php echo $row['nama_halaman']; ?>" >
			                                    ...
			                                    <div class="form-group">
			                                        ...
			                                        <textarea name="isi_halaman" class="ckeditor" id="editor1"><?php echo $row['isi_halaman']; ?></textarea>
			                                        </div>
			                                    </div>
			                                    ...
			                                    <input type="hidden" name="kd_halaman" value="<?php echo $kd_halaman; ?>" />
